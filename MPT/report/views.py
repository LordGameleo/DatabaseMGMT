...
@login_required(login_url = '/account/login/')
def add(request):
    if request.user.has_perm('report.add_date'):
        ...
        if request.method == 'POST' :
            if  form1.is_valid() and form2.is_valid() and form3.is_valid() and form4.is_valid():
                ...
                berth_number = form3.cleaned_data['berth_number']
            ...
            form1 = DateForm()
            form2 = ShiftForm(initial ={'shift':1})
            form3 = VesselForm()
            form4 = SlotForm()
...
@login_required(login_url = '/account/login/')
def add_slot_views(request):
    shifts_hmc_1 = Shift.objects.filter(shift_end_flag = 0).filter(hmc_number = 1)
    shifts_hmc_2 = Shift.objects.filter(shift_end_flag = 0).filter(hmc_number = 2)
    return render(request,'add_slot_views.html',{ 'shifts_hmc_1': shifts_hmc_1, 'shifts_hmc_2' : shifts_hmc_2 })

@login_required(login_url = '/account/login/')
def add_slot(request,shift_id):
    if request.user.has_perm('report.add_date'):
        slot_form = SlotForm(request.POST)
        if Shift.objects.filter(id = shift_id):
            shift_data = Shift.objects.filter(id = shift_id)[0]
            # getting vessel data from shift object
            slots = shift_data.slot_set.all()
            slot = slots[0]
            vessel = slot.vessel_set.all()[0]
            vessel_cargo_existing = vessel.vessel_cargo
            vessel_name_existing = vessel.vessel_name
            berth_number_existing = vessel.berth_number

        else:
            return render(request, 'message.html',{'message':"Shif doesn't exist. Please select the shift option which has been started and hasn't been ended"})

        if request.method == 'POST' :
            if  slot_form.is_valid():
                time_slot_start = slot_form.cleaned_data['time_slot_start']
                time_slot_stop = slot_form.cleaned_data['time_slot_stop']
                reason = slot_form.cleaned_data['reason']
                remark = slot_form.cleaned_data['remark']
                ...
                if shift_data.shift_end_flag == 0:
                    ...
                    vessel_data = Vessel(
                                    slot = slot_data,
                                    vessel_name = vessel_name_existing,
                                    vessel_cargo = vessel_cargo_existing,
                                    berth_number = berth_number_existing
                                  )
                    vessel_data.save()
                    return render(request,'message.html',{'message':"Entry Saved!!!"})
                else:
                    return render(request,'message.html',{'message':"Shift has already been ended.You can't add data again now."})
        ...
        return render(request, 'add_slot.html', { 'slot_form': slot_form, 'shift' : shift_data,
            'vessel_name' : vessel_name_existing, 'vessel_cargo' : vessel_cargo_existing, 'berth_number' : berth_number_existing })
    else:
        return render(request,'message.html',{'message':"You don't have permison to add data."})
...
def add_shift_end(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            diesel_start_percentage = form.cleaned_data['diesel_start_percentage']
            ...
            form = ShiftEndForm()
    shift_to_be_ended = Shift.objects.filter(shift_end_flag = 0)
    return render(request, 'add_shift_end.html', { 'form': form, 'shift_to_be_ended' : shift_to_be_ended })

def modify_options(request):
    return render(request, 'modify_options.html',{})

def modify(request):
    if request.user.has_perm('report.delete_date'):
...
def modify_slot(request,slot_id):
    ...
    if request.user.has_perm('report.delete_date'):
        if request.method == 'POST':
            if slot_form.is_valid() and vessel_form.is_valid():
                time_slot_start = slot_form.cleaned_data['time_slot_start']
                time_slot_stop = slot_form.cleaned_data['time_slot_stop']
                reason = slot_form.cleaned_data['reason']
                remark = slot_form.cleaned_data['remark']

                vessel_name = vessel_form.cleaned_data['vessel_name']
                vessel_cargo = vessel_form.cleaned_data['vessel_cargo']
                berth_number = vessel_form.cleaned_data['berth_number']

                slot = Slot.objects.filter(id = slot_id)[0]
                vessel = slot.vessel_set.all()[0]
                if slot.shift.shift_end_flag == 0:
                    slot.time_slot_start = time_slot_start
                    slot.time_slot_stop = time_slot_stop
                    slot.reason = reason
                    slot.remark = remark
                    ...
                    vessel.vessel_cargo = vessel_cargo

                    vessel.save()

                else:
                    return render(request, 'message.html', {'message' : "Shift has already been ended"})

                return render(request, 'message.html',{'message':"Entry modified"})
            else:
                slot_form = SlotForm()
                vessel_form = VesselForm()
        slot = Slot.objects.filter(id = slot_id)[0]
        return render(request,'modify_slot.html',{ 'slot_form' : slot_form, 'vessel_form' : vessel_form, 'slot': slot})
    else:
        return render(request, 'message.html', {'message' : "You don't have permision to modify"})
